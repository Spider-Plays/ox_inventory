...
--- Shop "Pay with bank" uses ESX accounts (bank is not an inventory-synced account by default).

---@diagnostic disable-next-line: duplicate-set-field
function server.getFrameworkAccountBalance(inv, currency)
	if currency ~= 'bank' then return end
...
---@diagnostic disable-next-line: duplicate-set-field
function server.removeFrameworkAccountMoney(inv, currency, amount)
	if currency ~= 'bank' then return end
	local xPlayer = server.GetPlayerFromId(inv.id)
	if not xPlayer then return false end
	local account = xPlayer.getAccount('bank')
	if not account or account.money < amount then return false end
	xPlayer.removeAccount('bank', amount)
	return true
end
...
function server.syncInventory(inv)
	...
	if accounts then
		...
		-- If `bank` is in inventory:accounts but not held as items, GetAccountItemCounts leaves bank at 0 and ESX would wipe the real bank.
		if accounts.bank ~= nil then
			local b = player.getAccount('bank')
			if b then accounts.bank = b.money end
		end
