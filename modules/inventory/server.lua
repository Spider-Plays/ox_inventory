...
lib.callback.register('ox_inventory:swapItems', function(source, data)
	...
	if toInventory and (data.toType == 'newdrop' or fromInventory ~= toInventory or data.fromSlot ~= data.toSlot) then
		...
		if fromData then
			...
			if not UtilitySlots.validatePlayerMove(playerInventory, fromInventory, toInventory, data.fromSlot, data.toSlot, fromData, toData) then
				return false, 'utility_slot_wrong_item'
			end
