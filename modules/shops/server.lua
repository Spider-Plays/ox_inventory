...
local function shopCurrencyBalance(inv, currency)
	local fw = server.getFrameworkAccountBalance and server.getFrameworkAccountBalance(inv, currency)
	if type(fw) == 'number' then return fw end
	return Inventory.GetItemCount(inv, currency)
end

local function canAffordItem(inv, currency, price)
	local balance = shopCurrencyBalance(inv, currency)
...
local function removeCurrency(inv, currency, price)
	local fw = server.removeFrameworkAccountMoney and server.removeFrameworkAccountMoney(inv, currency, price)
	if fw == true then return true end
	if fw == false then return false end
	return Inventory.RemoveItem(inv, currency, price) and true or false
end
...
lib.callback.register('ox_inventory:buyItem', function(source, data)
	if data.toType == 'player' then
		...
		if fromData then
			...
			-- Optional NUI payment method (cash vs bank) when shop price is default money.
			-- Use Items() so `bank` works even if inventory:accounts convar was not updated (server.accounts alone is not enough).
			if type(data.overrideCurrency) == 'string' and data.overrideCurrency ~= '' then
				local override = data.overrideCurrency
				if (not fromData.currency or fromData.currency == 'money') and Items(override) then
					currency = override
				end
			end
			...
			if toData == nil or (fromItem.name == toItem?.name and fromItem.stack and table.matches(toData.metadata, metadata)) then
				...
				local utilRole = UtilitySlots.roleForSlot(data.toSlot)
				if utilRole and not UtilitySlots.itemAllowed(utilRole, fromItem.name) then
					return false, false, { type = 'error', description = locale('utility_slot_wrong_item') }
				end
