...
local function armourRemainingPercent(slot)
	if slot and slot.metadata and type(slot.metadata.armor) == 'number' then
		return math.max(0, math.min(100, math.floor(slot.metadata.armor)))
	end
	return 100
end

local function applyArmourFromSlot(data, slot)
	local pct = armourRemainingPercent(slot)
	ox_inventory:useItem(data, function(resp)
		if resp then
			SetPlayerMaxArmour(PlayerData.id, 100)
			SetPedArmour(cache.ped, pct)
		end
	end)
end

Item('armour', function(data, slot)
	applyArmourFromSlot(data, slot)
end)
