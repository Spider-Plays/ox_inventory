...
--- Body armour uses metadata.armor (0–100), not weapon-style durability.
local function normalizeArmorMetadata(metadata)
	local a = metadata.armor
	...
	if type(a) ~= 'number' then
		...
		metadata.armor = math.max(0, math.min(100, math.floor(a)))
	end
	return metadata
end
...
function Items.Metadata(inv, item, metadata, count)
	...
	if item.weapon then
		...
		elseif not next(metadata) then
			...
			elseif item.name == 'armour' or item.name == 'armor' then
				normalizeArmorMetadata(metadata)
			end
		end

		if item.name == 'armour' or item.name == 'armor' then
			normalizeArmorMetadata(metadata)
		end
...
function Items.CheckMetadata(metadata, item, name, ostime)
	...
	if name == 'armour' or name == 'armor' then
		normalizeArmorMetadata(metadata)
	end
