...
export const itemDurability = (metadata: any, curTime: number, itemName?: string) => {
  // sorry dunak
  // it's ok linden i fix inventory
  const lower = itemName?.toLowerCase();
  if (lower === 'armour' || lower === 'armor') {
    const a = metadata?.armor;
    if (typeof a === 'number' && !Number.isNaN(a)) {
      return Math.max(0, Math.min(100, Math.floor(a)));
    }
    return 100;
  }
