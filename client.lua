...
do
	...
	local chunk = LoadResourceFile(resource, 'config.lua')

	if not chunk then
		error(("missing required config file '@%s/config.lua'"):format(resource), 0)
	end

	local fn, err = load(chunk, ("@@%s/config.lua"):format(resource))
...
function client.openInventory(inv, data)
    ...
    local ui = shared.config and shared.config.ui
    if ui then
        SendNUIMessage({
            action = 'setUiConfig',
            data = ui
...
--- Progress/anim for body armour; drag equip/remove pass mode for the bar label.
---@param itemName string
---@param metadata table
---@param mode? 'equip'|'remove'|nil nil = same label as using the item from inventory
---@return boolean
local function runArmourVestProgress(itemName, metadata, mode)
	if itemName ~= 'armour' and itemName ~= 'armor' then return true end

	local sharedItem = Items[itemName]
	if not sharedItem or not sharedItem.client then return true end

	local item = sharedItem.client
	...
	local displayLabel = metadata.label or sharedItem.label
	local progressLabel
	if mode == 'equip' then
		progressLabel = locale('armour_equipping', displayLabel)
	elseif mode == 'remove' then
		progressLabel = locale('armour_removing', displayLabel)
	else
		progressLabel = item.label or locale('using', displayLabel)
	end
	...
	if success then
		if item.notification and not mode then
			lib.notify({ description = item.notification })
		end

		if item.status and client.setPlayerStatus and not mode then
			client.setPlayerStatus(item.status)
		end
	end
...
--- Vest armour 0–100 stored in item metadata; kept in sync with GetPedArmour while equipped in utility vest slot.
local function vestArmorPercentFromMetadata(metadata)
	if metadata and type(metadata.armor) == 'number' then
		return math.max(0, math.min(100, math.floor(metadata.armor)))
	end
	return 100
end

local function syncUtilityVestArmorFromInventory()
	if not client.utilitySlots or not PlayerData.loaded then return end
	local vestSlot = client.utilitySlots.vest
	local item = PlayerData.inventory[vestSlot]
	if item and item.name and (item.name == 'armour' or item.name == 'armor') and item.count and item.count > 0 then
		local pct = vestArmorPercentFromMetadata(item.metadata)
		SetPlayerMaxArmour(PlayerData.id, 100)
		SetPedArmour(playerPed, pct)
	else
...
local function updateInventory(data, weight)
	...
	if client.utilitySlots and changes[client.utilitySlots.vest] ~= nil then
		syncUtilityVestArmorFromInventory()
	end
end
...
CreateThread(function()
	...
	while true do
		Wait(350)
		if not PlayerData.loaded or not client.utilitySlots then goto continue end
		local vestSlot = client.utilitySlots.vest
		local item = PlayerData.inventory[vestSlot]
		if not item or not item.name or (item.name ~= 'armour' and item.name ~= 'armor') or not item.count or item.count < 1 then
			lastSynced = -1
			goto continue
		end
		local pedArmor = GetPedArmour(playerPed)
		local stored = vestArmorPercentFromMetadata(item.metadata)
		if pedArmor ~= stored then
			if pedArmor ~= lastSynced then
				pcall(function()
					lib.callback.await('ox_inventory:syncArmourMetadata', 200, vestSlot, pedArmor)
				end)
				lastSynced = pedArmor
			end
		else
...
--- Write current ped armour onto the vest item before logout/save (avoids losing % if the sync thread hasn't run).
function client.flushVestArmorMetadata()
	if not client.utilitySlots or not PlayerData.loaded then return end
	local vestSlot = client.utilitySlots.vest
	local item = PlayerData.inventory[vestSlot]
	if not item or not item.name or (item.name ~= 'armour' and item.name ~= 'armor') or not item.count or item.count < 1 then return end
	local pedArmor = GetPedArmour(cache.ped)
	pcall(function()
		lib.callback.await('ox_inventory:syncArmourMetadata', 500, vestSlot, pedArmor)
	end)
end
...
RegisterNetEvent('ox_inventory:setPlayerInventory', function(currentDrops, inventory, weight, player)
	...
	-- Default: last four slots = VEST, BACKPACK, PHONE, CASH (override only when all four convars > 0)
	local utilityCustom = utilityVest > 0 and utilityPack > 0 and utilityPhone > 0 and utilityCash > 0
	local utilitySlots = {
		vest = utilityCustom and utilityVest or (shared.playerslots - 3),
		backpack = utilityCustom and utilityPack or (shared.playerslots - 2),
		phone = utilityCustom and utilityPhone or (shared.playerslots - 1),
		cash = utilityCustom and utilityCash or shared.playerslots,
	}
	client.utilitySlots = utilitySlots
	syncUtilityVestArmorFromInventory()
...
RegisterNUICallback('giveItemNearby', function(data, cb)
	...
	if nearbyCount == 0 then
		return lib.notify({ type = 'error', description = locale('nobody_nearby') })
	end
	...
	lib.registerMenu({
		id = 'ox_inventory:givePlayerListNearby',
		title = 'Give item',
		options = giveList,
	}, function(selected)
	...
	return lib.showMenu('ox_inventory:givePlayerListNearby')
end)
...
RegisterNUICallback('lockControls', function(_, cb)
	-- Keep game input blocked for the whole session; search/typing uses NUI focus only.
	if invOpen then SetNuiFocusKeepInput(false) end
	cb(1)
end)
...
RegisterNUICallback('swapItems', function(data, cb)
	...
	if data.armourVestEquip and type(data.armourItemName) == 'string' then
		local fromSlot = tonumber(data.fromSlot)
		local md = (fromSlot and PlayerData.inventory[fromSlot] and PlayerData.inventory[fromSlot].metadata) or {}

		if not runArmourVestProgress(data.armourItemName, md, 'equip') then
			swapActive = false
			return cb(false)
		end
	end

	if data.armourVestRemove and type(data.armourItemName) == 'string' then
		local fromSlot = tonumber(data.fromSlot)
		local md = (fromSlot and PlayerData.inventory[fromSlot] and PlayerData.inventory[fromSlot].metadata) or {}

		if not runArmourVestProgress(data.armourItemName, md, 'remove') then
			swapActive = false
			return cb(false)
		end
	end
