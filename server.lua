...
local UtilitySlots = require 'modules.utility_slots.server'
require 'modules.discordlogs.server'
...
--- Sync GTA ped armour back onto the vest item in the utility slot (metadata.armor 0–100).
--- At 0%, the vest item is removed (destroyed armour).
lib.callback.register('ox_inventory:syncArmourMetadata', function(source, slot, armorPercent)
	local inv = Inventory(source)
	if not inv or not inv.player then return false end
	if type(slot) ~= 'number' or type(armorPercent) ~= 'number' then return false end
	if UtilitySlots.roleForSlot(slot) ~= 'vest' then return false end
	local data = inv.items[slot]
	if not data or (data.name ~= 'armour' and data.name ~= 'armor') then return false end
	armorPercent = math.max(0, math.min(100, math.floor(armorPercent + 0.5)))
	if armorPercent <= 0 then
		local itemDef = Items(data.name)
		local itemLabel = (data.metadata and data.metadata.label) or (itemDef and itemDef.label) or data.name
		local ok = Inventory.RemoveItem(inv, data.name, data.count, nil, slot) == true
		if ok then
			TriggerClientEvent('ox_lib:notify', source, {
				type = 'inform',
				description = locale('armour_destroyed', itemLabel),
			})
		end
		return ok
	end
	local md = table.clone(data.metadata or {})
	if md.armor == armorPercent then return true end
	md.armor = armorPercent
	Inventory.SetMetadata(inv, slot, md)
	return true
end)
...
lib.callback.register('ox_inventory:useItem', function(source, itemName, slot, metadata, noAnim)
	...
	if inventory and inventory.player then
		...
		if item and data and data.count > 0 and data.name == item.name then
			...
			-- After using body armour from the main grid, move it into the utility vest slot (same as dragging it there).
			if (item.name == 'armour' or item.name == 'armor') and inventory.player then
				local vestSlot = UtilitySlots.getUtilitySlots().vest
				if type(vestSlot) == 'number' and slot ~= vestSlot then
					local fromItem = inventory.items[slot]
					local toItem = inventory.items[vestSlot]
					if fromItem and fromItem.name == item.name and fromItem.count and fromItem.count > 0 then
						if UtilitySlots.validatePlayerMove(inventory, inventory, inventory, slot, vestSlot, fromItem, toItem) then
							Inventory.SwapSlots(inventory, inventory, slot, vestSlot)
							CreateThread(function()
								inventory:syncSlotsWithClients({
									{
										item = inventory.items[vestSlot] or { slot = vestSlot },
										inventory = inventory.id,
									},
									{
										item = inventory.items[slot] or { slot = slot },
										inventory = inventory.id,
									},
								}, true)
							end)
